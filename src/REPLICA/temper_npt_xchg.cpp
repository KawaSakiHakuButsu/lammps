...
TemperNPTXCHG::TemperNPTXCHG(LAMMPS *lmp) : Command(lmp) {}

/* ---------------------------------------------------------------------- */

TemperNPTXCHG::~TemperNPTXCHG()
{
  MPI_Comm_free(&roots);
...
void TemperNPTXCHG::command(int narg, char **arg)
{
  ...
  double press = utils::numeric(FLERR,arg[6],false,lmp);
  ...
  my_set_temp_press = universe->iworld;
  if (narg == 8) my_set_temp_press = utils::inumeric(FLERR,arg[7],false,lmp);
  if ((my_set_temp_press < 0) || (my_set_temp_press >= universe->nworlds))
  ...
  // create static list of set pressures
  // allgather tempering arg "press" across root procs
  // bcast from each root to other procs in world

  set_press = new double[nworlds];
  if (me == 0) MPI_Allgather(&press,1,MPI_DOUBLE,set_press,1,MPI_DOUBLE,roots);
  MPI_Bcast(set_press,nworlds,MPI_DOUBLE,0,world);

  // create world2temp only on root procs from my_set_temp_press
  ...
  if (me == 0) {
    MPI_Allgather(&my_set_temp_press,1,MPI_INT,world2temp,1,MPI_INT,roots);
    for (int i = 0; i < nworlds; i++) temp2world[world2temp[i]] = i;
  }
  MPI_Bcast(temp2world,nworlds,MPI_INT,0,world);

  // if restarting tempering, reset temp and press target of Fix to current my_set_temp_press

  if (narg == 8) {
    double new_temp = set_temp[my_set_temp_press];
    whichfix->reset_target(new_temp);
    double new_press = set_press[my_set_temp_press];
    whichfix->reset_press(new_press);
  }

  // setup tempering runs

  int i,which,partner,swap,partner_set_temp_press,partner_world;
  double pe,pe_partner, delr,boltz_factor,new_temp, new_press;
  ...
  for (int iswap = 0; iswap < nswaps; iswap++) {
    ...
    // partner_set_temp_press = which set temp and press I am partnering with for this swap

    if (which == 0) {
      if (my_set_temp_press % 2 == 0) partner_set_temp_press = my_set_temp_press + 1;
      else partner_set_temp_press = my_set_temp_press - 1;
    } else {
      if (my_set_temp_press % 2 == 1) partner_set_temp_press = my_set_temp_press + 1;
      else partner_set_temp_press = my_set_temp_press - 1;
    }
    ...
    partner = -1;
    if (me == 0 && partner_set_temp_press >= 0 && partner_set_temp_press < nworlds) {
      partner_world = temp2world[partner_set_temp_press];
    ...
    // Acceptance criteria changed versus temper command for NPT ensemble
      if (me_universe < partner) {
        delr = (pe_partner - pe)*(1.0/(boltz*set_temp[my_set_temp_press])
                                  - 1.0/(boltz*set_temp[partner_set_temp_press]))
          + (set_press[my_set_temp_press]/nktv2p/(boltz*set_temp[my_set_temp_press])
                         - set_press[partner_set_temp_press]/nktv2p/(boltz*set_temp[partner_set_temp_press]))*(vol_partner - vol);
        boltz_factor = -delr;
      ...
      if (me_universe < partner)
        fprintf(universe->uscreen,"SWAP %d & %d: yes = %d,Ts = %d %d, PEs = %g %g, Bz = %g %g, vol = %g %g\n",
               me_universe,partner,swap,my_set_temp_press,partner_set_temp_press,
    ...
    if (swap) scale_velocities(partner_set_temp_press,my_set_temp_press);

    // if my world swapped, all procs in world reset temp target of Fix

    if (swap) {
      new_temp = set_temp[partner_set_temp_press];
      whichfix->reset_target(new_temp);
      new_press = set_press[partner_set_temp_press];
      whichfix->reset_press(new_press);
    }

    // update my_set_temp_press and temp2world on every proc
    // root procs update their value if swap took place
    // allgather across root procs
    // bcast within my world

    if (swap) my_set_temp_press = partner_set_temp_press;
    if (me == 0) {
      MPI_Allgather(&my_set_temp_press,1,MPI_INT,world2temp,1,MPI_INT,roots);
...
void TemperNPTXCHG::scale_velocities(int t_partner, int t_me)
{
...
void TemperNPTXCHG::print_status()
{
