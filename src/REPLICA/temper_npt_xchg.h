...
#ifdef COMMAND_CLASS
// clang-format off
CommandStyle(temper/npt_xchg,TemperNPTXCHG);
// clang-format on
#else

#ifndef LMP_TEMPERNPTXCHG_H
#define LMP_TEMPERNPTXCHG_H

#include "command.h"

namespace LAMMPS_NS {

class TemperNPTXCHG : public Command {
 public:
  TemperNPTXCHG(class LAMMPS *);
  ~TemperNPTXCHG() override;
 ...
 private:
  ...
  int my_set_temp_press;     // which set temp and press I am simulating
  double *set_temp;    // static list of replica set temperatures
  double *set_press;    // static list of replica set pressures
